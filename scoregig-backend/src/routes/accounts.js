...
accounts.patch("/me/profile", auth(), (req, res) => {
  ...
  db.prepare("UPDATE users SET bio = ?, experience = ?, city = ?, games_worked = ?, sports = ?, phone = ?, notifications_enabled = ?, sms_consent = ?, sms_consent_at = ?, sms_opted_out = ? WHERE id = ?")
    .run(bio, experience, city, gamesWorked, JSON.stringify(sports), phone || null, notificationsEnabled ? 1 : 0, smsConsent, smsConsentAt, smsOptedOut, req.user.id);
  res.json({ bio, experience, city, gamesWorked, sports, phone: phone || "", notificationsEnabled, smsConsent: Boolean(smsConsent), smsOptedOut: Boolean(smsOptedOut) });
});
